...
export const Profile: React.FC = () => {
  ...
  const { items, removeFromWishlist, executeWishlistItem } = useWishlistStore();
  ...
  const handleExecuteWishlist = async (name: string) => {
    try {
      ...
      // You could also use fetchStockData with the saved options
      console.log('Executed wishlist item:', response);
    } catch (error) {
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-gray-600 mt-1">
                          <span className="font-medium">{item.options.symbol}</span>
                          <span className="mx-2">•</span>
                          <span>{item.options.function}</span>
                          {item.options.interval && (
                            <>
                              <span className="mx-2">•</span>
                              <span>{item.options.interval}</span>
                            </>
                          )}
                        </div>
                      </div>
